...
def addToBlockedWebsites():
    ...
    with open(host_path, "r+") as host_file:
        ...
        if (not is_valid_address([website_address])) and (
            not is_valid_ip(website_address)
        ):
            label = tk.Label(
                advancedWindow,
                text=website_address + " is not valid.",
                font="Lato 12 bold",
            )
        ...
        if (
            is_valid_address([website_address])
            or is_valid_ip(website_address)
            and website_address not in file_content
        ):
            host_file.seek(0)
...
def UnBlocker():
    ...
    for website in sorted(UnWebsite):
        ...
        if not website in file_content:
            ...
            label = tk.Label(
                root, text="Unblocked", font="Lato 12 bold", fg="green", bg="#132824"
            )
            label.place(x=250, y=200)
            label.after(1000, label.destroy)
...
def Blocker():
    ...
    with open(host_path, "r+") as host_file:
        ...
        for website in sorted(Website):
            if (not is_valid_address([website])) and (not is_valid_ip(website)):
                label = tk.Label(
                    root, text=website + " is not valid.", font="Lato 12 bold"
                )
                label.place(x=250, y=200)
                label.after(1000, label.destroy)
                break
            ...
            if website in file_content:
                ...
                pass
            if (
                is_valid_address([website])
                or is_valid_ip(website)
                and website not in file_content
            ):
                host_file.seek(0)
                data = host_file.read(100)
                if len(data) > 0:
                ...
                label = tk.Label(
                    root, text="Blocked", font="Lato 12 bold", fg="red", bg="#132824"
                )
                label.place(x=250, y=200)
                label.after(1000, label.destroy)
                website_lists = Websites.delete("1.0", tk.END)
...
class Error(tk.Toplevel):
    def __init__(self, message, window):
        ...
        tk.Button(
            self, command=combine_funcs(window.destroy, self.destroy), text="OK"
        ).grid(row=1, column=0)
        self.lift()
        self.grab_set()
...
def createHelpWindow():
    ...
    if helpCounterWindow < 2:
        ...
        tk.Label(
            helpWindow,
            text='Block Website : enter the website proceeding with "www" or IP. \nEx.: www.example.com \nIf you want to add multiple links separate them with ",".',
            font="Lato 10 bold",
            foreground="white",
            background="#222",
            compound="center",
        ...
        tk.Label(
            helpWindow,
            text='Unblock Website : the same like in "Block Website"',
            font="Lato 10 bold",
            foreground="white",
            background="#222",
            compound="center",
        ...
        tk.Label(
            helpWindow,
            text="Restore to default : removes all blocked websites",
            font="Lato 10 bold",
            foreground="white",
            background="#222",
            compound="center",
        ...
        tk.Label(
            helpWindow,
            text="To effectively block website type: 'www.youradress.com'\n and 'youradress.com'",
            font="Lato 10 bold",
            foreground="white",
            background="#222",
            compound="center",
...
def createAdvancedWindow():
    ...
    if advancedCounterWindow < 2:
        ...
        scrollbar = ttk.Scrollbar(
            advancedWindow, orient=tk.VERTICAL, command=tree.yview
        )
        ...
        bBtn = tk.Button(
            ...
            borderwidth=0,
            command=combine_funcs(addToBlockedWebsites),
        )
        bBtn.place(x=275, y=5)
        bBtn.image = lockImg

        unblockBtn = tk.Button(
            advancedWindow,
            text="UnBlock",
            font="Lato 12 bold",
            image=unlockImg,
            ...
            activebackground="#222",
            borderwidth=0,
            command=addToUnblockedWebsites,
        )
...
if is_admin():
    ...
    maindir = (
        os.getcwd()
    )  # Put all the files including images next to WebsiteBlocker.exe so application can load them
    ...
    Websites = tk.Text(
        root, font="Lato 10", height="2", width="40", wrap=tk.WORD, padx=5, pady=5
    )
    ...
    AdvancedBtn = tk.Button(
        root,
        text="Editor",
        command=createAdvancedWindow,
        image=advancedimg,
        background="#222",
        activebackground="#222",
        borderwidth=0,
